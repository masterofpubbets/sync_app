...
#Region "Data Methods" '
            ...
            Public Function ReturnCommand(ByVal sql As String) As SqlClient.SqlCommand
                ...
                Return cmd
            End Function
            Public Sub ExportDataGridData(ByRef grd As DataGridView, ByVal SaveedExcelPath As String, ByVal SheetName As String)
                Try
                    Dim x As New EAMS.OfficeAutomation.Excels
                    x.SaveBulkDataGrid(grd)
                    x.SetRange("A1", 1, grd.Columns.Count)
                    x.FormateRange(Color.FromArgb(75, 130, 199), Color.White, 9, True, 25)
                    x.CellBorder(Color.White, 2)
                    x.SetRange("A2", grd.Rows.Count + 1, grd.Columns.Count)
                    x.FormateRange(Color.White, Color.Black, 9, True, 25)
                    x.CellBorder(Color.Gray, 2)
                    x.Refresh()
                    x.Save(SaveedExcelPath, SheetName)
                    x.Close()
                    x = Nothing
                    GC.Collect()
                ...
                End Try
            End Sub
            Public Overloads Sub ExportDataToExcel(ByVal ExcelFilePath As String, ByVal QueryToExport As String, ByVal SheetName As String, Optional ByVal Timeout As Integer = 15, Optional Ranges() As st_ExcelRange = Nothing)
                Try
                    ...
                    DA.Fill(DT)
                    If DT.Columns.Contains("tbl_id") Then
                        DT.Columns.Remove("tbl_id")
                    End If
                    ...
                    For Each dr As DataRow In DT.Rows
                        For iny = 1 To DT.Columns.Count
                            If dr.Item(iny - 1).ToString <> "" Then ex.Write(inx + 1, iny, dr.Item(iny - 1).ToString)
                    ...
                    If Not IsNothing(Ranges) Then
                        For Rinx As Integer = 0 To Ranges.Count - 1
                            ...
                            ex.FormateRange(Ranges(Rinx).BackColor, Ranges(Rinx).ForeColor, Ranges(Rinx).FontSize, Ranges(Rinx).IsBold, Ranges(Rinx).ColumnWidth)
            ...
            Public Overloads Sub ExportDataToExcel(ByVal ExcelFilePath As String, ByRef DT As DataTable, ByVal SheetName As String, Optional Ranges() As st_ExcelRange = Nothing)
                Try
                    Dim ex As New EAMS.OfficeAutomation.Excels
                ...
                End Try
            End Sub
            Public Sub ExportDataHeaderToExcel(ByVal ExcelFilePath As String, ByVal QueryToExport As String, ByVal SheetName As String, Optional Ranges() As st_ExcelRange = Nothing)
                Try
                    Dim ex As New EAMS.OfficeAutomation.Excels
                    Dim DA As New SqlClient.SqlDataAdapter(QueryToExport, DB)
                    Dim DT As New DataTable
                    ...
                    If Not IsNothing(Ranges) Then
                        For Rinx As Integer = 0 To Ranges.Count - 1
                            ex.SetRange(Ranges(Rinx).ColName, 1 + 1, Ranges(Rinx).RangeColCount)
                            ex.FormateRange(Ranges(Rinx).BackColor, Ranges(Rinx).ForeColor, Ranges(Rinx).FontSize, Ranges(Rinx).IsBold, Ranges(Rinx).ColumnWidth)
                            ex.CellBorder(Ranges(Rinx).BorderColor, Ranges(Rinx).LineWeight)
                        Next
                    End If
            ...
            Public Overloads Function ExcutResult(ByVal SQL As String, Optional ByRef RowEffected As Integer = 0) As String
                ...
                Catch ex As Exception
                    RaiseEvent err(ex.Message)
                    ErrorLog(ex.Message, ex.Message)
                End Try
                Return temp
            End Function
...
#Region "Database Methods"
            ...
            Private Sub SaveLog(ByRef q As String)
                ...
                End Try
            End Sub
            Public Function ExcuteNoneResultFromFile(ByVal Path As String, Optional ByVal Timeout As Integer = 0)
                ...
                SQLServerCmd.CommandType = System.Data.CommandType.Text
                Try
                    If InStr(Sql, "go", CompareMethod.Text) > 0 Then
                        temp = Split(Sql, "go",, CompareMethod.Text)
                    ...
                    Return True
                Catch ex As Exception
                    ErrorLog(Path, ex.Message)
                    RaiseEvent err(ex.Message)
                    Return False
                End Try
                Return False
            End Function
            Public Function ExcuteNoneResult(ByVal Query As String, Optional ByVal Timeout As Integer = 0) As Boolean
                SQLServerCmd.Connection = DB
                SQLServerCmd.CommandType = System.Data.CommandType.Text
                ...
                Return False
            End Function
            Private Sub ErrorLog(ByVal er As String, ByVal Description As String)
                Try
            ...
            Public Sub DeAttachDatabase(ByVal DBName As String)
                Try
                    ...
                    Dim tmpSql As String = String.Format("sp_detach_db N'{0}' , N'true'", DBName)
                ...
                End Try
            End Sub
            Public Overloads Function GetGridItemSelected(ByRef grd As DataGridView, ByVal SelectedRowIndex As Integer, ByVal ColumnIndex As Integer) As String
                Try
                    ...
                    If grd.SelectedCells(0).Selected = False Then
                        Return ""
                    Else
                        Return (grd.Rows(SelectedRowIndex).Cells(ColumnIndex).Value)
                    End If
                Catch ex As Exception
                    Return ""
                End Try
                Return ""
            End Function
            Public Overloads Function GetGridItemSelected(ByRef grd As DataGridView, ByVal ColumnIndex As Integer) As String
                Try
                    If grd.Rows.Count = 0 Then
                        Return ""
                    End If
            ...
            Public Overloads Sub FillGrd(ByRef grd As DataGridView, ByVal SQL As String, Optional ByVal Timeout As Integer = 30)
                ...
                Try
                    ...
                    If Not IsNothing(DT) Then grd.DataSource = DT
                Catch ex As Exception
                End Try
            End Sub
...
#Region "Properties"
            ...
            Public Function GetServerInfo() As String
                ...
                For inx = 0 To dt.Columns.Count - 1
                    tmp &= String.Format("{0}: {1}{2}", dt.Columns(inx).ColumnName, dt.Rows(0).Item(inx), vbCrLf)
                Next
                Return tmp
            End Function
#End Region
            ...
            Public Sub GetCrConnectionInfo(ByVal ReportSource As String, ByRef v As CrystalDecisions.Windows.Forms.CrystalReportViewer)
                ...
                For Each CrTable In CrTables
                    ...
                    If Me.Pass <> "" Then
                        ...
                        crConnectionInfo.Password = Me.Pass
...
10:
            ...
            Public Sub CrQuickExportToPDF(ByVal ReportSource As String, ByVal PDFFullPath As String, Optional Parameter As String = "")
                Try
                    Dim crtableLogoninfo As New CrystalDecisions.Shared.TableLogOnInfo()
                    Dim crConnectionInfo As New CrystalDecisions.Shared.ConnectionInfo()
                    Dim CrExportOptions As New CrystalDecisions.Shared.ExportOptions
...
#End Region
        ...
        Public Class MSAccessTools
            Private DB As New System.Data.OleDb.OleDbConnection
            Public DataBaseName As String = ""
            Public DataBaseLocation As String = "."
            Public UserName As String = ""
            Public Pass As String = ""
            Private AccessCmd As New System.Data.OleDb.OleDbCommand
            Public Event Connnected()
            Public Event Disconnected()
            Public Event err(ByVal errName As String)
            Public Event ExportingComplete()
...
#Region "Database Methods"
            Public Sub ExportDataHeaderToExcel(ByVal ExcelFilePath As String, ByVal QueryToExport As String, ByVal SheetName As String, Optional Ranges() As st_ExcelRange = Nothing)
                Try
                    Dim ex As New EAMS.OfficeAutomation.Excels
                    Dim DA As New OleDb.OleDbDataAdapter(QueryToExport, DB)
                    Dim DT As New DataTable
                    DA.Fill(DT)
                    If DT.Columns.Contains("tbl_id") Then
                        DT.Columns.Remove("tbl_id")
                    ...
                    Next
                    If IsNothing(Ranges) Then
                        Dim R(1) As EAMS.DataBaseTools.MSAccessTools.st_ExcelRange
                        R(0) = New EAMS.DataBaseTools.MSAccessTools.st_ExcelRange
                        R(1) = New EAMS.DataBaseTools.MSAccessTools.st_ExcelRange
                        '--------------------------------
                        R(0).ColName = "A1"
                        R(0).RangeColCount = DT.Columns.Count
                        R(0).BackColor = Color.FromArgb(75, 130, 199)
            ...
            End Sub
            Public Overloads Sub Fill(ByRef lst As ListBox, ByVal SQL As String)
                Dim DA As New System.Data.OleDb.OleDbDataAdapter(SQL, DB)
                Dim DT As New DataTable, x As Integer
                DA.Fill(DT)
                lst.Items.Clear()
                For x = 0 To DT.Rows.Count - 1
            ...
            End Function
            Public Sub Connect()
                Dim AccCon As String = ""
                If Pass = "" Then
                    AccCon = String.Format("Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0};", Trim(DataBaseLocation), DataBaseName)
                Else
                    AccCon = String.Format("Provider=Microsoft.ACE.OLEDB.12.0;Data Source={2}; Jet OLEDB:Database Password={3};", UserName, DataBaseName, Trim(DataBaseLocation), Pass)
                End If
                Try
                    If (DB.State = System.Data.ConnectionState.Open Or DB.State = System.Data.ConnectionState.Connecting) Then
                        DB.Close()
                    End If
                    DB.ConnectionString = AccCon
                    Do Until DB.State = ConnectionState.Open
                        DB.Open()
                    Loop
                    RaiseEvent Connnected()
...
#Region "Shape"
            ...
            <Description("Set the Radius of All four Corners to the Same Value")>
            <Category("Edges")>
...
#Region "Properties"
            ...
            <Description("Defines the first Color of the gradient Colors"), Category("Button Colours"), DefaultValue(GetType(Color), "Control")>
            Property ButtonGradientColor1() As Color
                ...
                End Set
            End Property
            <Description("Defines the second Color of the gradient Colors"), Category("Button Colours"), DefaultValue(GetType(Color), "Control")>
            ...
            <Description("Defines the Third Color of the gradient Colors"), Category("Button Colours"), DefaultValue(GetType(Color), "Control")>
            ...
            <Description("Defines the Brush Style"), Category("Button Colours"), DefaultValue(1)>
            ...
            <Description("Defines the Linear Gradient Direction"), Category("Button Colours"), DefaultValue(1)>
            ...
            <Description("Defines the X value of the Focus Point when the Brush Style is Set to Path. Value between 0 and 1."), Category("Button Colours"), DefaultValue(0.5)>
            Property FocusPointX() As Decimal
                ...
                End Set
            End Property
            <Description("Defines the Y value of the Focus Point when the Brush Style is Set to Path. Value between 0 and 1."), Category("Button Colours"), DefaultValue(0.5)>
            ...
            <Description("Defines the X value of the Center Point when the Brush Style is Set to Path and Focus Points set to 0."), Category("Button Colours"), DefaultValue(0.5)>
            Property CenterPointX() As Decimal
                ...
                End Set
            End Property
            <Description("Defines the Y value of the Center Point when the Brush Style is Set to Path and Focus Points set to 0."), Category("Button Colours"), DefaultValue(0.5)>
            ...
            <Description("Defines the Position value of the 2nd Color.Value between 0 and 1."), Category("Button Colours"), DefaultValue(0.5)>
            ...
            <Category("Image"),
           Description("Defines the Button Image"),
           DefaultValue("(None)")>
            ...
            <Category("Image"),
           Description("Defines the X Coordinate of the Upper Left Corner of the Image Location"),
           DefaultValue(10)>
            ...
            End Property
            <Category("Image"),
           Description("Defines the Y Coordinate of the Upper Left Corner of the Image Location"),
           DefaultValue(10)>
            ...
            <Category("Image"), Description("Defines the Size of the Image"), DefaultValue("16, 16")>
            ...
            <Category("Image"), Description("Defines the Image Rotation Angle. Value between 0 and 359."), DefaultValue(0)>
            Public Property ImageRotationAngle() As Short
                ...
                End Set
            End Property
            <Category("Image"), Description("Defines if the Image Will Animate or not."), DefaultValue("False")>
            Public Property ImageAnimation() As Boolean
                ...
                End Set
            End Property
            <Category("Appearance"), Description("Sets the Text Style."), DefaultValue("Regular")>
...
#End Region
            ...
            Private Function DrawImageRotatedAroundCenter(ByVal center As Point, ByVal img As Image,
            ...
            Public Function DrawTriangle(ByVal rec As RectangleF, ByVal str As String)
                ...
                Select Case str
                    Case "Up"
                        pts = New PointF() {
                            New PointF(CSng(rec.Width / 2), rec.Y),
                            New PointF(rec.Width, rec.Y + rec.Height),
                            New PointF(rec.X, rec.Y + rec.Height)}
                    Case "Down"
                        pts = New PointF() {
                            New PointF(rec.X, rec.Y),
                            New PointF(CSng(rec.Width / 2), rec.Y + rec.Height),
                            New PointF(rec.X + rec.Width, rec.Y)}
                    Case "Left"
                        pts = New PointF() {
                            New PointF(rec.X, CSng(rec.Y + (rec.Height / 2))),
                            New PointF(rec.Width, rec.Y),
                            New PointF(rec.Width, rec.Y + rec.Height)}
                    Case "Right"
                        pts = New PointF() {
                            New PointF(rec.X, rec.Y),
                            New PointF(rec.Width, CSng(rec.Y + (rec.Height / 2))),
                            New PointF(rec.X, rec.Y + rec.Height)}
                End Select
            ...
            Function DimBlenedColor(ByVal c1 As Color, ByVal c2 As Color, ByVal c3 As Color, ByVal DimDegree As Integer, ByVal Brush As BrushType, ByVal gp As GraphicsPath) As Brush
                ...
                Select Case mBrush
                    Case BrushType.Linear
                        Dim br As LinearGradientBrush = New LinearGradientBrush(New Rectangle(0, 0, Me.Width, Me.Height),
                ...
                Select Case mStyle
                    ...
                    Case Text_Style.Block
                        If s = GradientButtonStates.Disabled Then
                            Dim I As Short
                            For I = 5 To 0 Step -1
                                g.DrawString(Me.Text, Me.Font, Brushes.Gray,
                        ...
                        Else
                            Dim I As Short
                            For I = 5 To 0 Step -1
                                g.DrawString(Me.Text, Me.Font, Brushes.Black,
                                New RectangleF(rec.X - I, rec.Y - I, rec.Width, rec.Height), StrFor)
                            Next
                            g.DrawString(Me.Text, Me.Font, Brushes.SeaShell, rec, StrFor)
                        End If
...
#Region "GUI Properties"
            <System.ComponentModel.Description("The text shown in the header of the imagecombo"), System.ComponentModel.DefaultValue(GetType(String), "No Items")>
            Property HeaderText() As String
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Show/Hide the Tool Tips"), System.ComponentModel.DefaultValue(GetType(Boolean), "True")>
            Property ShowItemToolTips() As Boolean
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Indicate weather the control shold draw right to left"), System.ComponentModel.DefaultValue(GetType(Boolean), "False")>
            Property WriteRightToLeft() As Boolean
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Allow item to be selected when the mouse over it"), System.ComponentModel.DefaultValue(GetType(Boolean), "False")>
            Property HoverSelecting() As Boolean
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Allow item to appear as a hyperlink when the mouse over it"), System.ComponentModel.DefaultValue(GetType(Boolean), "False")>
            Property HotTracking() As Boolean
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Display grid lines around items show only when it in small images mode"), System.ComponentModel.DefaultValue(GetType(Boolean), "False")>
            Property GridLines() As Boolean
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the List fore color for the ImageCombo"), System.ComponentModel.DefaultValue(GetType(Color), "0,0,0")>
            Property ListForeColor() As Color
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the List back color for the ImageCombo"), System.ComponentModel.DefaultValue(GetType(Color), "255,255,255")>
            Property ListBackColor() As Color
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the Header color for the ImageCombo"), System.ComponentModel.DefaultValue(GetType(Color), "112,128,144")>
            Property HeaderColor() As Color
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the Text Header color for the ImageCombo"), System.ComponentModel.DefaultValue(GetType(Color), "255,255,255")>
            Property HeaderTextColor() As Color
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the Small ImageList for the ImageCombo")>
            Public Property SmallImagList() As ImageList
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the Big ImageList for the ImageCombo")>
            Public Property BigImagList() As ImageList
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the Big ImageList for the ImageCombo"), System.ComponentModel.DefaultValue(GetType(e_ImageType), "Small")>
            Public Property ImageSize() As e_ImageType
                ...
                End Set
            End Property
            <System.ComponentModel.Description("Sets the Height for the ImageCombo"), System.ComponentModel.DefaultValue(GetType(Integer), "200")>
...
#Region "Remove Chars"
            ...
            Private Function RemoveSingleQoute(ByVal Value As String) As String
                ...
                Return t.ToString
            End Function
            Private Function RemoveWindowsReservedChars(ByVal Value As String) As String
...
#Region "Methods"
            ...
            Public Function RemoveString(ByVal Value As String, ByVal RemoveType As enChars) As String
                Select Case RemoveType
                    ...
                    Case enChars.CommaChars
                        Return RemoveCommaChar(Value)
                    Case enChars.WindowsReservedChars
                        Return RemoveWindowsReservedChars(Value)
                    Case enChars.Null + enChars.Space + enChars.HelpChar + enChars.UpperChar + enChars.LowerChar
                        Dim temp As String
                        temp = RemoveNull(Value)
                        temp = RemoveSpace(temp)
...
#End Region
        ...
        Public Class FileEncoding
            ...
            Public Sub EncryptOrDecryptFile(ByVal strInputFile As String, ByVal Password As String, ByVal Direction As CryptoAction)
                ...
                Try 'In case of errors.
                    ...
                    If Direction = CryptoAction.ActionEncrypt Then
                        RaiseEvent Info("Encryption Complete" + Wrap + Wrap +
                    ...
                    Else
                        'Update the user when the file is done.
                        RaiseEvent Info("Decryption Complete" + Wrap + Wrap +
                               "Total bytes processed = " +
                                lngBytesProcessed.ToString)
                ...
                Catch When Err.Number = 53 'if file not found
                    RaiseEvent Info("Please check to make sure the path and filename" +
                ...
                Catch
                    ...
                    If Direction = CryptoAction.ActionDecrypt Then
                        ...
                        RaiseEvent Info("Please check to make sure that you entered the correct" +
                                "password.")
                    Else
                        Dim fileDelete As New FileInfo(strOutputFile)
                        fileDelete.Delete()
    ...
    Namespace Mailling
        ...
        Public Class SMTP
            ...
            '*******************************************************************************
            Public Shared Sub BrainDeadSimpleEmailSend(ByVal strFrom As String,
            ...
            Public Shared Function QuickiEMail(ByVal strFrom As String,
                              ...
                              Optional ByVal SSLUsername As String = vbNullString,
                              Optional ByVal SSLPassword As String = vbNullString,
            ...
            Public Shared Function SendEMail(ByVal strFrom As String,
                              ...
                              Optional ByVal strCC As String = vbNullString,
                              Optional ByVal strBcc As String = vbNullString,
                              Optional ByVal strAttachments As String = vbNullString,
                              Optional ByVal smtpPort As Integer = 25,
                              Optional ByVal usesSSL As Boolean = False,
                              Optional ByVal SSLUsername As String = vbNullString,
                              Optional ByVal SSLPassword As String = vbNullString,
...
KeyFound:
    ...
    Namespace MSOffice
        Public Class Excel
            ...
            Public Function GetSheetData(ByVal WorkBookPath As String, ByVal SheetName As String, Optional filter As String = "") As DataTable
                ...
                Try
                    ...
                    Else
                        cmd = New System.Data.OleDb.OleDbDataAdapter(String.Format("select * from [{0}$] where " & filter, SheetName), cn)
                    End If
                    cn.Open()
                    cmd.Fill(ds)
                    cn.Close()
                ...
                Return Nothing
            End Function
            Public Function GetSheetData(ByVal WorkBookPath As String, ByVal SheetName As String, ByVal SelectCols As String, Optional filter As String = "") As DataTable
                Dim cn As System.Data.OleDb.OleDbConnection
                Dim cmd As System.Data.OleDb.OleDbDataAdapter
                Dim ds As New DataSet
                Dim dt As New DataTable
                ...
                Try
                    ...
                    ' Select the data from Sheet1 of the workbook.
                    If filter = "" Then
                        cmd = New System.Data.OleDb.OleDbDataAdapter(String.Format("select " & SelectCols & " from [{0}$]", SheetName), cn)
                    Else
                        cmd = New System.Data.OleDb.OleDbDataAdapter(String.Format("select " & SelectCols & " from [{0}$] where " & filter, SheetName), cn)
                    End If
                    cn.Open()
                    cmd.Fill(ds)
                    cn.Close()
    ...
    Namespace OfficeAutomation
        Public Class Excels
            ...
            Public Sub FormateRange(ByVal IntColor As Color, ByVal FontColor As Color, Optional FontSize As Byte = 11, Optional IsBold As Boolean = False, Optional ColumnWidth As Byte = 0)
                Try
            ...
            Public Sub SaveBulkDataGrid(ByRef grd As DataGridView)
                ...
                With objSheet
                    ...
                    'Clipboard.SetData(DataFormats.Text, grd.GetClipboardContent())
                    .Paste()
                    Clipboard.Clear()
                End With
            End Sub
            Public Sub AddDatatable()
                objBook.Connections.Add2("bbb", "m", "Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=True;Initial Catalog=TREICAJAZAN05;Data Source=.;Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Use Encryption for Data=False;Tag with column collation when possible=False", "select * from tblelecablelist", Microsoft.Office.Interop.Excel.XlCmdType.xlCmdSql)
